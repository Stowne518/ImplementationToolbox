...
void genericDataImport(bool* p_open, Sql& sql, AppLog& log, std::string dir)
{
	...
	static std::vector<std::string> valid_rows;
	static std::unordered_set<std::string> existing_values;
	...
	static std::future<std::vector<std::string>> csv_columns_future;
	static std::future<std::vector<std::string>> csv_rows_future;
	static std::future<std::vector<std::vector<std::string>>> destination_columns_future;
	static std::future<std::vector<std::string>> insert_rows_future;
	static std::future<void> process_data_future;
	static std::future<void> insert_builder_future;
	static bool
		is_fetching_rows = false,
		is_loading_destination_columns = false,
		is_csv_loading = false,
		is_csv_loading_data = false,
		is_data_processing = false,
		is_loading_inserts = false,
		is_data_processed = false,
		is_building_inserts = false;
	...
	log.logStateChange("isGettingRows", is_fetching_rows);
	log.logStateChange("isLoadingColumns", is_loading_destination_columns);
	log.logStateChange("isLoadingCSV", is_csv_loading);
	log.logStateChange("isLoadingData", is_csv_loading_data);
	log.logStateChange("isProcessingData", is_data_processing);
	log.logStateChange("isLoadingInserts", is_loading_inserts);
	log.logStateChange("dataProcessed", is_data_processed);
	log.logStateChange("isBuildingInserts", is_building_inserts);
	log.logStateChange("filepath.empty()", filepath.empty());

	// OOP Class initialization testing
	static auto destination_table = new DestinationTable();
	...
	if (Button("Load CSV", (filepath.empty() || loaded_csv)))  // Only enable button if filepath is !empty OR we have loaded_csv already
	{
		try
		{
			...
			csv_rows_future = std::async(
				std::launch::async,
				getRows,
				std::filesystem::path(dir + "DataImport\\" + filepath)
			);
			is_csv_loading = true;    // Flag the process of loading CSV has started
			is_csv_loading_data = true;
	...
	if (is_csv_loading)
	{
		...
		if (std::chrono::duration_cast<std::chrono::milliseconds>(now - last_check).count() > 250)
		{
			last_check = now;
			if (csv_columns_future.valid() && csv_columns_future.wait_for(std::chrono::milliseconds(0)) == std::future_status::ready)
			{
				try
				{
					// Retrieve the result
					source_columns = csv_columns_future.get();
			...
			if (csv_rows_future.valid() && csv_rows_future.wait_for(std::chrono::milliseconds(0)) == std::future_status::ready)
			{
				try
				{
					// Retrieve the result
					data_rows = csv_rows_future.get();
			...
			if (!csv_columns_future.valid() && !csv_rows_future.valid())
			{
				is_csv_loading = false; // Reset the loading flag
				is_csv_loading_data = false;
	...
	if (Button("Load Columns", (load_columns || table_name.empty())))
	{
		try
		{
...

			{
				...
				sql_connection_string = sql._GetConnectionString();  // After locking sqlMutex get connection string value to protect from changes
				curr_table_name = table_name;                   // After locking sqlMutex get table name to protect from changes while processing
			}
			// Start the asynchronous operation
			destination_columns_future = std::async(
				std::launch::async,
				[&sql, curr_table_name]() {
					//sql._SetConnectionString();

					return sql.getTableColumns(curr_table_name);
				}
			);
			is_loading_destination_columns = true; // Indicate that the columns are being loaded
		}
		catch (const std::exception& e)
		{
	...
	if (is_loading_destination_columns)
	{
		// Check if future is ready without blocking
		if (destination_columns_future.valid() && destination_columns_future.wait_for(std::chrono::milliseconds(0)) == std::future_status::ready)
		{
			try
			{
				// Retrieve the result
				destination_columns = destination_columns_future.get();
				...
				for (int i = 0; i < destination_columns[0].size(); /*i++*/)                                                     // First layer of vectors are the containers for each complete column set
				{
					if (destination_columns[0][i] == "adduser" || destination_columns[0][i] == "addtime")                       // If the column name is adduser or addtime remove it and each matching index for the schema information
					{
						...
						addtime = true;
						destination_table->set_addtime(true);
						destination_table->set_adduser(true);
					}
					else
					{
				...
				for (int j = 0; j < destination_columns[0].size(); j++)
				{
					...
					destination_table->add_column(destination_columns[0][j], destination_columns[1][j], destination_columns[2][j], destination_columns[3][j]);
				...
				load_columns = true;
				is_loading_destination_columns = false; // Reset the loading flag
			}
			catch (const std::exception& e)
			{
	...
	// Window containing display options
	if (ImGui::BeginPopup("WindowOptions", ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysAutoResize))
	{
	...
	if (!is_csv_loading && confirm_mapping && !is_csv_loading_data && !is_data_processed && !is_data_processing)
	{
		// Start asynchronous thread to process data
		process_data_future = std::async(
			std::launch::async,
			processData,
			std::ref(data_rows),
		...
		is_data_processing = true; // Indicate that data processing has started
	}
	if (is_data_processing)
	{
		ImGui::Text("Cleaning up data and processing for editing...");
		if (process_data_future.valid() && process_data_future.wait_for(std::chrono::milliseconds(0)) == std::future_status::ready)
		{
			is_data_processing = false; // Reset the loading flag
			is_data_processed = true;       // Set a compeletion flag since we access this function via a menu button
			log.AddLog("[INFO] Data processing completed successfully.\n");
		}
	}

	if (ImGui::BeginTabBar("GenericImportTabBar", ImGuiTabBarFlags_None))
	{
		if (ImGui::BeginTabItem("1. ColumnMapping"))
		{
			ImGui::BeginChild("Column Mapping", ImVec2(ImGui::GetContentRegionAvail()),
			...
			if (ImGui::BeginMenuBar())
			{
				...
				ImGui::Text("Status:");
				if (is_loading_destination_columns || is_csv_loading)
				{
			...
			ImGui::EndChild(); // End Column mapping child window
			ImGui::EndTabItem();
		}
		if (ImGui::BeginTabItem("2. Data Staging"))
		{
			...
			ImGui::EndChild(); // End Data staging child window
			ImGui::EndTabItem();
		}
		if (ImGui::BeginTabItem("3. Insert Builder"))
		{
			...
			{
				if (!is_building_inserts)
				{
					insert_builder_future = std::async(
						std::launch::async,
						buildInsertQuery,
						std::ref(table_name),
						std::ref(insert_rows),
					...
					is_building_inserts = true;
				}
			}
			if (is_building_inserts &&
				insert_builder_future.valid() &&
				insert_builder_future.wait_for(std::chrono::milliseconds(0)) == std::future_status::ready)
			{
				is_building_inserts = false;
			...
			if (confirm_data && !is_building_inserts)
			{
			...
			if (ImGui::BeginTabBar("Insert"))
			{
				if (ImGui::BeginTabItem("Insert Rows"))
				{
					...
					if (insert_rows.size() >= 1 && !is_building_inserts && !running)
					{
			...
			ImGui::EndChild(); // End insert builder child window
			ImGui::EndTabItem();
		}
		ImGui::EndTabBar();
	}
	...
	if (mapoverview)
	{
		...
		ImGui::SetNextWindowSize(ImVec2(200, ImGui::GetContentRegionAvail().y - 275));
		ImGui::Begin("Mapping Overview", &mapoverview,
			ImGuiChildFlags_AlwaysAutoResize |
			ImGuiWindowFlags_MenuBar
		);
